...
- ✅ **C1 (DONE):** `DisplayPrefs.monthlyBudgetUSD` (General pane field) + `ContextSnapshot.budgetTier` (worst-of monthly-run-rate-vs-budget and hottest 5h limit %, tiers 0.8/1.0 and 50/80) + a trailing menubar-title dot (`AppDelegate.budgetSuffix`: yellow at warn, red at critical, hidden when calm). `Agent.totalCost30d` now parsed. Swift typecheck + General-pane screenshot validated; menubar dot color eyeballed live. (The popover already showed burn/ETA/5h%/reset.)
