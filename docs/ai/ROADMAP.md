...
5. **E1 (port half)** — fold `usage_signal.py` into `context-bar-core`, incremental + golden-test-pinned so cost fidelity can't regress. Slices:
   ...
   - ⏳ **Slice 4 — online/host + wiring** (needed before flipping the daemon off Python, else the menubar regresses):
     1. `collect_others` probes (gemini-cli / aider / zsh-history; `llm` sqlite optional).
     2. statusline-snapshot read (local JSON, live context %).
     3. LiteLLM **live pricing fetch** + 24h disk cache (HTTP via `ureq`) + dynamic table threading through `match_pricing` (keep the golden on `FALLBACK`).
     4. Anthropic usage API (account 5h/7d %) + Codex transcript rate-limits — cross-platform credential read (`~/.claude/.credentials.json`; keychain on mac). Codex app-server JSON-RPC probe can stay deferred (degrades gracefully).
     5. **`context.json` parity**: serialize `by_week`/`by_month`/`by_project` with Python's keys (`week`/`month`/`project`), and a `source` the consumers accept (today they gate on `source == "python3"` — update the CLI + Swift to a positive availability check).
     6. **Wire** `collect_native` → Rust, full-snapshot real-data diff, menubar screenshot, then delete `usage_signal.py` from the native path (wasm Zed extension keeps it — or later shells out to the `context-bar` binary). Struct-mirror drift + python3 native dep gone → unblocks A1/A2.
