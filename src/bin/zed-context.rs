//! Standalone CLI for the Zed Context Pilot engine.
//!
...
//! run once to refresh `.zed-context/{hud.md,AGENT.md,state.json,...}`, or
...
//!
//! Example: `zed-context watch 30 .` (or set up launchd to keep this alive).
...
use zed_context_pilot::context_engine::{self, ContextSnapshot};
use zed_context_pilot::git_signal::{self, ChangeSummary, CommitSummary, GitSignals};
use zed_context_pilot::hud;
use zed_context_pilot::state_writer;
use zed_context_pilot::usage_signal;
...
fn print_help() {
    eprintln!(
        "zed-context — Claude Code + Codex CLI usage HUD for any repo\n\n\
         USAGE:\n\
         \x20   zed-context hud          [worktree_root]   refresh repo .zed-context/hud.md\n\
         \x20   zed-context snapshot     [worktree_root]   refresh full repo artifacts\n\
         \x20   zed-context global                         write ~/.zed-context/hud.md\n\
         \x20   zed-context watch        [secs] [root]     loop per-repo (default 30s)\n\
         \x20   zed-context watch-global [secs]            loop ~/.zed-context/hud.md\n\n\
         Pin `~/.zed-context/hud.md` in Zed for an always-visible cross-project HUD.\n"
    );
}
...
fn run_hud(root: Option<PathBuf>) -> i32 {
    match refresh(root) {
        Ok((root, snapshot)) => {
            ...
            eprintln!("\nHUD written to {}/.zed-context/hud.md", root.display());
            0
        }
        Err(error) => {
...
fn run_snapshot(root: Option<PathBuf>) -> i32 {
    match refresh(root) {
        Ok((root, _)) => {
            println!("artifacts refreshed in {}/.zed-context/", root.display());
            0
        }
        Err(error) => {
...
fn run_watch_global(secs: u64) -> i32 {
    eprintln!("zed-context watch-global: every {secs}s. Ctrl-C to stop.");
    loop {
        match refresh_global() {
...
fn refresh_global() -> Result<PathBuf, String> {
    ...
    let dir = PathBuf::from(&home).join(".zed-context");
    ...
    let html = zed_context_pilot::detail_html::render(&usage);
...
fn format_usage_row(label: &str, usage: &zed_context_pilot::usage_signal::AgentUsage) -> String {
...
fn run_watch(root: Option<PathBuf>, secs: u64) -> i32 {
    eprintln!("zed-context watch: every {secs}s. Ctrl-C to stop.");
    loop {
        match refresh(root.clone()) {
            Ok((root, _)) => eprintln!(
                "[{}] refreshed {}/.zed-context/hud.md",
                now_local(),
                root.display()
            ),
            Err(error) => eprintln!("[{}] refresh error: {error}", now_local()),
