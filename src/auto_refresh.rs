...
//! `.zed-context/AGENT.md` plus `state.json` appear without them running a
//! command.
//!
...
//!
//! Idempotency: a fresh `.zed-context/state.json` (younger than
...
fn try_refresh(worktree: &zed::Worktree) -> Result<(), String> {
    ...
    let state_path = root.join(".zed-context").join("state.json");
