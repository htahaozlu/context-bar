...
- **Plan-value projection** — the Cost tab projects a monthly metered-API cost from the last 30 days and compares it to your actual Anthropic plan price (e.g. *"≈ $8,302/mo — about 42× your Max 20× plan"*). No other tracker (CLI or menubar) reframes API-equivalent cost against the plan you actually pay for — the exact question a subscriber facing forced metering asks.
- **Cache-savings insight** — surfaces the net USD prompt caching saved vs. paying full input price (e.g. *"Prompt caching saved $47,139 in the last 30 days"*). Every tool tracks cache tokens; none monetize the framing.
- **30-day cost-trend sparkline** with peak $/day, for passive at-a-glance burn awareness a CLI can't provide.
- `CONTEXTBAR_SCREENSHOT_SIZE=WxH` env to size the detail window for marketing captures (capture-only; no effect in normal use).
